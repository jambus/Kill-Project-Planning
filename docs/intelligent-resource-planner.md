...
4. **严格排期窗口约束 (Strict Scheduling Window)**：用户的排期时间选择区间（如 4月至6月）即为绝对物理边界。系统的实际推算如果越过该窗口的最后一天（如 6月30日），则会触发「硬截断」甚至直接放弃分配。若项目因资源不足无法在该窗口内完工，剩余的人天将作为「未排缺口」进行预警，而绝不跨月溢出。
