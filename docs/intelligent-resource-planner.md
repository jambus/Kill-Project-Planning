...
graph TD
    ...
    BatchAI --> HardDeduction[<b>JS 像素级扣减</b><br/>在矩阵中寻找首个可用日期<br/>⚠️ 仅校验首日容量 (见 CD-001)]
...
3.  **收敛收割循环 (Convergence Loops)**：最后的收割阶段不再是一次性尝试。系统会自动执行最多 3 轮的贪心补排（relaxed 模式，跳过技能匹配），每一轮都会拿着上一轮释放的碎余资源重新寻找缺口，直到分配结果达到数学意义上的收敛。**注意**：当前无提前终止阈值，技能不匹配时仍会消耗 3 次 AI 调用（参见 QL-004）。
4.  **排期完整性回滚 (All-or-Nothing Rollback)**：在核心批次处理后，系统强制执行事务检查。若项目仅单阶段（dev 或 test）获得分配而另一阶段完全未启动，则回滚该项目释放占位资源。**注意**：当前回滚条件未评估剩余排期窗口是否仍有机会完成另一阶段（参见 QL-001）。
...
4. **严格排期窗口约束与防死循环 (Strict Scheduling Window & Early Exit)**：用户的排期时间选择区间（如 4月至6月）即为绝对物理边界。
   * 系统的实际推算如果越过该窗口的最后一天（如 6月30日），`endDate` 会被截断到 `scheduleMaxDate`。**注意**：当前实现仅截断日期，不验证截断后的有效工作天数是否仍能满足 `finalMd` 需求（参见 CD-002）。
...
#### 3.3.8 已知技术债务与优化方向 (Known Technical Debt & Optimization Roadmap)

> 以下内容基于 2026-05-10 排期引擎代码审查，标记了当前架构中已知的优化点和正确性缺陷，供后续迭代参考。

##### 正确性缺陷

| ID | 问题 | 文件位置 | 描述 |
|----|------|----------|------|
| CD-001 | `findEarliestFitDate` 只校验首日容量 | `SchedulingContext.tsx:126` | 分配开始时的 `available >= percentage` 只检查了第一天。后续日期的容量可能在分配周期内不足，导致 `generateResourceCalendar` 隐性超分配，被 `Math.max(0, ...)` 掩盖。 |
| CD-002 | `endDate` 越界后静默截断 | `SchedulingContext.tsx:198` | `endDate > scheduleMaxDate` 时直接截断到窗口边界，不验证有效工作天数是否仍满足 `finalMd`。分配记录被创建但实际人天不足。 |

##### 性能债务

| ID | 问题 | 文件位置 | 描述 |
|----|------|----------|------|
| PF-001 | 日历矩阵反复全量重建 | `SchedulingContext.tsx:49` | `runAudit` 每次调用都遍历整个时间窗口为所有资源重建日历。PASS 1 每批 2 次 + PASS 2 1 次 + PASS 3 每轮 2 次 + `applySuggestions` 对每个建议 1 次，复杂度 O(N·D·R) 被重复放大。 |
| PF-002 | 全量审计粒度太粗 | `SchedulingContext.tsx:96` | 每次审计都计算所有项目缺口 + 所有资源闲置，即使大量实体数据未变化。 |

##### 排期质量

| ID | 问题 | 位置 | 描述 |
|----|------|------|------|
| QL-001 | PASS 2 回滚未评估剩余窗口 | `SchedulingContext.tsx:242-248` | dev 部分完成但 test 未启动即整体回滚，不检查窗口内是否仍有机会完成另一阶段。 |
| QL-002 | AI Prompt 指令冲突 + 日期信息非结构化 | `ai.ts` | 指令 3（用完所有空闲）与指令 4（最少拆分）在资源紧张时冲突；`scheduleSummary` 以自然语言而非结构化 JSON 传入，增加 LLM 推理负担。 |
| QL-003 | AI 最少 3 天约束硬逻辑未落地 | `SchedulingContext.tsx:189` | Prompt 要求最低 3 天，但 `Math.max(1, ...)` 只保证了 ≥1。 |
| QL-004 | PASS 3 无提前终止阈值 | `SchedulingContext.tsx:255` | 3 轮贪心收割在资源不匹配时仍会浪费 AI 调用。应加入最小分配量阈值。 |
| QL-005 | 可用窗口碎片化 | `SchedulingContext.tsx:82` | `dailyAvailable` 频繁波动时产生大量细碎窗口，增加 LLM 推理负担。 |

##### 鲁棒性

| ID | 问题 | 位置 | 描述 |
|----|------|------|------|
| RB-001 | AI 中途失败无事务回滚 | `SchedulingContext.tsx:201` | `applySuggestions` 逐条写入 IndexedDB，后续批次失败时已分配数据已持久化。虽然 `clear()` 在下次排期时执行，但用户体验差。 |
| RB-002 | 内存与 DB 状态一致性脆弱 | `SchedulingContext.tsx:200` | `currentAllocations` 和 `db.allocations.add()` 解耦写入，单条失败会导致分歧。 |
