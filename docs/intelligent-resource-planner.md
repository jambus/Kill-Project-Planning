...
1. **优先保障机制**：在 AI 接手任何高优业务项目（PASS 1）之前，系统会率先通过代码硬逻辑（JS Deterministic Logic）直接扣减配置好的每月运维研发与测试人天，确保运维需求得到 100% 的资源保障。
...
3. **零 Token 消耗**：PASS 0 完全基于本地策略演算，无需请求大模型 API，在实现复杂业务意图的同时保证了极高的运行效率和零成本。
4. **可视化大盘区分**：分配给运维任务的人天在全局看板中会以绿色的 `[运维] Product Name` 独立标识，直观区分于正常的业务需求开发。
