...
#### 3.3.8 调度引擎优化需求 (Scheduling Engine Optimization Requirements)

> 来源：2026-05-09 代码审查，以下为调度引擎的待改进需求，按优先级分组。

##### P0 — 性能
1. **调度过程中的矩阵状态应在内存中增量维护**：每条 AI 建议被应用后，系统只更新受影响资源的日历槽位，而不重新对所有资源执行全量审计。全量审计仅在每个 PASS 结束时执行一次，以保证结果正确性。
2. **节假日/调休日的查找应为 O(1)**：系统在初始化时将节假日与调休工作日列表转换为集合结构，确保逐日遍历时的判断代价为常数级，而非随列表长度线性增长。

##### P1 — 准确性
3. **资源日历矩阵在同一调度会话中应只构建一次**：`runAudit` 与日期推算逻辑应共享同一套矩阵，不应对同一资源重复构建独立的 calendar 副本。
4. **测试准入日期应为该项目所有开发分配时间跨度的中点**：系统应取该项目全部开发分配中最早开始日与最晚结束日的时间中点，作为测试人员的最早介入日期，而非直接使用开发的最早开始日。
5. **AI 返回的分配建议应做合法性校验**：系统在采用 AI 返回结果前，需过滤掉 `projectId`/`resourceId` 缺失、`allocatedMd` 小于 1、`allocationPercentage` 超出合理范围的非法条目，并记录警告日志。

##### P2 — 健壮性
6. **"停止排期"操作应能立即取消正在进行中的 AI 网络请求**：当前停止逻辑仅在异步边界检查标志位，无法中断耗时较长的 fetch 调用，停止响应存在延迟。
7. **PASS 2 完整性回滚应覆盖"开发严重欠配且测试完全未排"的场景**：当一个项目的开发分配不足预估工时 50%、且测试完全未获分配时，该项目也应触发资源回滚，释放资源给后续 PASS 重新分配。
8. **项目缺口与资源闲置的内部计算应保留完整精度**：缺口数值在调度计算过程中应以浮点数传递，仅在写入数据库和 UI 展示时进行四舍五入，避免连续取整导致的边界偏差。

##### P3 — 成本与低频性能
9. **发送给 AI 的资源上下文信息应进行精简**：`scheduleSummary` 字段内容过长时应截断，只保留最近数个空闲时间窗口的摘要，以降低每次 AI 请求的 Token 消耗。
10. **工作日集合应在排期开始时预先生成**：系统在启动调度前，一次性构建排期窗口内的完整工作日集合，后续所有工作日判断直接查表，避免重复逐日计算。
